...
We used two replicates of CD44<sup>+</sup> T cell data sets from Ciucci et al. 2019 [^2] as an example to demonstrate the use of `robustSingleCell`. The analysis requires at least 8G of memory on *slurm* [^3] high performance computing workload manager (for example, you can start by requesting `srun --pty -p <partition> --mem=8G -t 1:00:00 bash` to start an interactive session). We first download the raw 10X data from GEO using `GEOquery`, which can be obtained using the following command if not already installed:
...
more than two fold change modularity difference relative to the shuffled analysis.
...
<figcaption> Fig 3. Violin plot of cell cycle score.</figcaption>
</figure>
...
[^2]: Ciucci, Thomas, et al. "The Emergence and Functional Fitness of Memory CD4+ T Cells Require the Transcription Factor Thpok." _Immunity_ 50.1 (2019): 91-105.
