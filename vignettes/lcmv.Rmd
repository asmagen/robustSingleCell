...
We used two replicates of CD44<sup>+</sup> T cell data sets from Ciucci et al. 2018 [^1] as an example to demonstrate the use of `MagenSingleCell`. The analysis requires about 4GB to 8GB of memory on *slurm* [^2] high performance computing workload manager. We first download the raw 10X data from GEO using `GEOquery`, which can be obtained using the following command if not already installed:
