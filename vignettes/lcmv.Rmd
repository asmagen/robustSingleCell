...
Next, we identify highly variable genes on which we base the PCA and clustering analyses. We also compute the activation of gene sets of interest, such as cell cycle genes, which can be used for confounder correction using the `regress` parameter in the PCA analysis. Figure 1 shows the mitochondrial score vs UMI number pre and post filtering. Figure 2 shows the violin plot for the cell cycle score. 
...
<figcaption> Fig 1. Mitochondrial score vs UMI for pre (top) and post (bottom) filtered cells. </figcaption>
</figure>

<figure>
<img src="figs/violin.pdf"> 
<figcaption> Fig 2. Cell cycle score violin plot </figcaption>
</figure>

`PCA` function performed PCA after a simulation analysis of shuffled data to determine the appropriate number of PCs. Figure 3 and 4 visualizes the first two PCs, their rotation and loadings. 
...
<figcaption> Fig 3. First two PCs and corresponding rotation </figcaption>
</figure>
...
<figcaption> Fig 4. Heatmap visualization of the loadings of the first two PCs </figcaption>
</figure>

We perform clustering analysis for a range of clustering resolutions. The analysis is repeated multiple times over shuffled data to estimate the appropriate clustering resolution and control for false discovery of clusters, as described in *Magen et al 2018*. 
...
The clusters are then annotated by customized set of marker genes which the users should adapt to their own data. `summarize` function performs differential expression analysis, computes tSNE and visualizes the results in the analysis folder. Figure 5 shows the tSNE visualization of the clusters. 
...
<figure>
<img src="figs/tsne.pdf" width="500"> 
<figcaption> Fig 5. t-SNE plot colored by clusters </figcaption>
</figure>

We can also visualize the heatmap of canonical T cells markers as follows for initial evaluation.
...
<figcaption> Fig 6. Heatmap of the canonical markers for T cell subsets </figcaption>
</figure>
...
robustSingleCell:::plot.simple.heatmap(LCMV2, name = "canonical", markers = canonical_genes,
...
We assessed the similarity between pairs of clusters as described in *Magen et al 2018* and identify reproducible clusters across the two replicates. Figure 7 shows the correlation heatmap of clusters across replicates
...
<figcaption> Fig 7. Correlation between pairs of clusters in the two datasets. </figcaption>
</figure>

Finally, the Pearson correlations between clusters are visualized as a heatmap ordered by hierarchical clustering, shown in Figure 8.
...
<figure>
<img src="figs/cor_FC.pdf" width = "900"> 
<figcaption> Fig 8. Correlation among all the clusters in the two datasets </figcaption>
</figure>
