...
visualize.cluster.similarity.stats <- function(environment, similarity) {
    ...
    print(gplots::heatmap.2(similarity.matrix, col = color.palette, key = T, cexRow = 3.5,
        ...
        Rowv = T, Colv = T, dendrogram = "both", notecol = "white", main = "", margins = c(40, 40)))  # cellnote = round(similarity.matrix, 1), notecex = 2, main = 'Pearson Correlation Between Cluster FC'
    grDevices::dev.off()
}
